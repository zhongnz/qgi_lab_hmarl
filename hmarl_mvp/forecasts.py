...
@dataclass
class NoiselessForecaster:
    """Noiseless forecaster: tiles the current queue forward as a constant.

    Unlike :class:`GroundTruthForecaster`, this has no model of in-transit
    vessels or future arrivals — it simply repeats the present queue snapshot.
    """
    ...
    def predict(self, ports: list[PortState]) -> tuple[np.ndarray, np.ndarray]:
        """Deterministic noiseless forecast from current realized queue state."""
        current_q = np.array([p.queue for p in ports], dtype=float)[:, None]
